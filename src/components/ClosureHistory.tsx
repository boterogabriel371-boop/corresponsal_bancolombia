...
export default function ClosureHistory({ onPrintHistoric, onExportHistoricPDF }: ClosureHistoryProps) {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6 items-stretch min-h-[calc(100vh-140px)]" id="closure-history-module">
      
      {/* LEFT COLUMN: FILTERS & CARD LIST */}
      <div className="w-full lg:w-[450px] flex flex-col gap-4 shrink-0" id="history-sidebar">
        
        {/* Filters Panel */}
        <div className="bg-white rounded-2xl border border-slate-200 p-4 shadow-xs flex flex-col gap-3" id="filters-container">
          <h4 className="text-sm font-extrabold text-slate-800 flex items-center gap-1.5" id="filters-title">
          ...
          <div className="grid grid-cols-2 gap-2" id="filters-date-inputs">
            <div className="flex flex-col gap-1" id="filter-start-group">
              <label className="text-[10px] font-extrabold text-slate-500 uppercase tracking-widest" htmlFor="start-date-input">
                Desde
              </label>
              <input
                type="date"
                ...
                className="w-full text-xs font-bold bg-slate-50 border border-slate-200 focus:border-indigo-600 focus:ring-1 focus:ring-indigo-600 focus:outline-none rounded-lg p-2 text-slate-700 font-mono"
              />
            </div>
            <div className="flex flex-col gap-1" id="filter-end-group">
              <label className="text-[10px] font-extrabold text-slate-500 uppercase tracking-widest" htmlFor="end-date-input">
                Hasta
              </label>
              <input
                type="date"
          ...
          <div className="flex items-center justify-between gap-2 mt-1 pt-3 border-t border-slate-100" id="filters-toggles">
            <button
              onClick={() => setSortOrder((order) => (order === "desc" ? "asc" : "desc"))}
              className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs text-slate-700 hover:text-indigo-650 font-bold bg-slate-100 hover:bg-slate-200 rounded-lg border border-slate-200 transition-all cursor-pointer"
              id="sort-toggle-btn"
            >
        ...
        <div className="flex-1 overflow-y-auto max-h-[500px] lg:max-h-[calc(100vh-270px)] pr-1 flex flex-col gap-2.5" id="history-cards-scroll">
          {sortedClosures.length === 0 ? (
            <div className="bg-white rounded-2xl border border-slate-200 p-8 text-center text-slate-400 flex flex-col items-center justify-center gap-3" id="cards-empty-state">
              <BookOpen size={28} className="text-slate-300" />
              <div>
                <p className="text-sm font-bold text-slate-600">No se encontraron cierres</p>
              ...
              return (
                <div
                  ...
                  className={`p-4 rounded-xl border transition-all duration-200 cursor-pointer text-left relative overflow-hidden group ${
                    isSelected
                      ? "bg-slate-900 border-transparent text-white shadow-md transform translate-x-1"
                      : "bg-white hover:bg-slate-50 border-slate-200 hover:border-slate-300 text-slate-800 shadow-3xs"
                  }`}
                ...
                >
                  <div className="flex items-center justify-between pointer-events-none" id={`card-heading-${c.id}`}>
                    <div className="flex items-center gap-2" id={`card-date-badge-${c.id}`}>
                      <Calendar size={13} className={isSelected ? "text-slate-300" : "text-slate-400"} />
                      <span className="text-xs font-extrabold capitalize">{dateStr}</span>
                    </div>
                    <span className={`text-[9px] font-mono font-bold opacity-70`} id={`card-time-badge-${c.id}`}>
                      {formatLocalTime(c.createdAt)}
                    </span>
                  </div>

                  <div className="grid grid-cols-2 gap-4 mt-3 pt-3 border-t border-slate-100/10 pointer-events-none" id={`card-sums-grid-${c.id}`}>
                    <div id={`card-sum-caja-${c.id}`}>
                      <p className={`text-[10px] font-bold uppercase tracking-wider ${isSelected ? "text-slate-300" : "text-slate-400"}`}>
                        Total Caja
                      </p>
                    ...
                    <div id={`card-sum-cash-${c.id}`}>
                      ...
                      <p className={`text-sm font-extrabold font-mono tracking-tight mt-0.5 ${isSelected ? "text-slate-200" : "text-gray-600"}`}>
      ...
      <div className="flex-1 lg:min-w-[450px]" id="history-details-pane">
        {selectedClosure ? (
          <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-xs flex flex-col gap-6 sticky top-24" id="details-box">
            
            {/* Detail Heading */}
            <div className="flex items-start justify-between pb-4 border-b border-slate-150" id="details-heading">
              <div id="details-heading-text">
                <p className="text-[10px] font-extrabold text-[#4f46e5] uppercase tracking-widest" id="details-heading-sub">
                  Resumen de Archivo
                </p>
                <h3 className="text-xl font-extrabold text-slate-900 mt-1 capitalize" id="details-heading-main">
                  Cierre: {formatLocalDate(selectedClosure.createdAt)}
                </h3>
                <div className="flex items-center gap-1.5 mt-1.5 text-xs text-slate-500 font-mono font-bold" id="details-meta">
              ...
              <div className="flex items-center gap-1.5" id="details-actions">
                <button
                  ...
                  className="p-2 text-slate-500 hover:text-indigo-650 hover:bg-slate-100 rounded-lg transition-colors cursor-pointer"
                ...
                <div className="h-6 w-px bg-slate-200 mx-1" id="det-divider"></div>
                <button
                  onClick={() => {
                ...
                <button
                  ...
                  className="ml-1 p-1.5 text-slate-400 hover:text-slate-600 rounded-md hover:bg-slate-100 transition-colors cursor-pointer"
            ...
            <div className="bg-slate-900 text-white rounded-xl p-4.5 flex items-center justify-between shadow-2xs" id="details-grand-total">
              <div>
                <p className="text-[10px] font-extrabold text-slate-300 uppercase tracking-widest" id="det-grand-total-label">
                  Total Consolidado en Caja
                </p>
                <h4 className="text-3xl font-black font-mono tracking-tight mt-1 text-white" id="det-grand-total-val">
                  {formatCOP(selectedClosure.grandTotal)}
                </h4>
              </div>
              <div className="px-3 py-1.5 rounded-lg bg-slate-800 border border-slate-700 text-xs text-slate-300 font-bold font-mono uppercase tracking-wider" id="det-grand-total-tag">
                HISTÓRICO
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 max-h-[350px] overflow-y-auto pr-1" id="details-sections">
              ...
              <div className="flex flex-col gap-2.5" id="details-cash-section">
                ...
                <div className="border border-slate-200 rounded-xl overflow-hidden divide-y divide-slate-100" id="det-cash-table">
                  {selectedClosure.denominations.filter((d) => d.quantity > 0).length === 0 ? (
                    <div className="p-4 text-center text-xs text-slate-400 font-bold" id="det-cash-empty">
                    ...
                    selectedClosure.denominations
                      ...
                      .map((d) => (
                        <div key={d.denomination} className="flex items-center justify-between px-3.5 py-2.5 text-xs text-slate-700" id={`det-item-${d.denomination}`}>
                          <div className="flex items-center gap-2" id={`det-item-info-${d.denomination}`}>
                            <span className="font-mono font-bold bg-slate-100 border border-slate-200 px-1.5 py-0.5 rounded text-indigo-700 font-mono">
              ...
              <div className="flex flex-col gap-2.5" id="details-balances-section">
                ...
                <div className="border border-slate-200 rounded-xl p-3.5 flex flex-col gap-3.5 bg-slate-50/40" id="det-balances-list">
                  ...
                  <div className="flex items-center justify-between text-xs font-bold" id="det-bal-bancolombia">
                    ...
                    <span className={`font-mono text-slate-800 ${selectedClosure.bancolombiaBalance > 0 ? "font-bold text-indigo-605" : "opacity-60"}`}>
                  ...
                  <div className="flex items-center justify-between text-xs font-bold" id="det-bal-cupo">
                    ...
                    <span className={`font-mono text-slate-800 ${selectedClosure.bancolombiaCredit > 0 ? "font-bold text-indigo-650" : "opacity-60"}`}>
                  ...
                  <div className="flex items-center justify-between text-xs font-bold" id="det-bal-tks">
                    ...
                    <span className={`font-mono text-slate-800 ${selectedClosure.tksBalance > 0 ? "font-bold text-indigo-650" : "opacity-60"}`}>
                  ...
                  <div className="flex items-center justify-between text-xs font-bold" id="det-bal-ptm">
                    ...
                    <span className={`font-mono text-slate-800 ${selectedClosure.ptmBalance > 0 ? "font-bold text-indigo-650" : "opacity-60"}`}>
            ...
            <div className="flex flex-col gap-2.5" id="details-adjustments-section">
              ...
              <div className="flex flex-col gap-1.5" id="det-adjustments-list">
                {selectedClosure.adjustments.length === 0 ? (
                  <p className="text-xs text-slate-550 italic bg-slate-50 px-3 py-2 rounded-xl border border-slate-100" id="det-adjustments-empty">
            ...
            <div className="flex flex-col gap-2" id="details-observations-section">
              ...
              <div className="bg-slate-50 border border-slate-200 p-3.5 rounded-xl text-xs text-slate-705 font-bold leading-relaxed" id="det-obs-box">
                {selectedClosure.observations ? (
                  <p className="whitespace-pre-line" id="det-obs-text">{selectedClosure.observations}</p>
                ) : (
                  <p className="italic text-slate-400 font-medium" id="det-obs-empty">Sin observaciones ni novedades registradas.</p>
                )}
              </div>
            </div>
          ...
          <div className="bg-white rounded-2xl border border-dashed border-slate-300 p-12 text-center text-slate-400 flex flex-col items-center justify-center gap-4.5 h-[300px] lg:h-full lg:min-h-[400px]" id="details-stub">
            <div className="p-3 bg-slate-50 rounded-full border border-slate-100" id="stub-icon-circle">
              <FileText size={32} className="text-slate-450 animate-bounce" />
            </div>
            <div>
              <p className="text-sm font-bold text-slate-700">Ningún archivo abierto</p>
              <p className="text-xs text-slate-550 max-w-sm mt-1 mx-auto font-medium leading-relaxed pb-2">
